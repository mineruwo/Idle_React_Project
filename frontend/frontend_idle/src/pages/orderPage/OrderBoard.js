...
const OrderBoard = () => {
  ...
  /* 선택된 오더의 등록일(createdAt)을 상단에 표시 */
  ...
  return (
    <div className="page-wrap">
                ...
                <div className="info-grid">
                  ...
                  <div>
                    ...
                    <div className="sub-value">
                      {prettyPacking(o.packingOptions ?? o.packingOption)}
                    </div>
                  </div>
                  <div>
      ...
      {/* 오른쪽: 상세 패널 */}
      <aside ref={panelRef} className={`detail-area ${panelOpen && selected ? "open" : ""}`}>
        <div className="detail-header">
          ...
          {/* 등록일(createdAt) 표시 */}
          <div className="today">{createdAtStr}</div>
        </div>
          ...
          <>
            <div className="badge-row">
              {isAssigned(selected, assignment) ? (
                <span className="badge">
                  배정: {selected?.assignedDriverNickname || `Driver #${assignedDriver}`} /{" "}
                  {Number(confirmedPrice ?? 0).toLocaleString()}원
                </span>
              ) : (
            ...
            <section className="section">
              ...
              <div className="row">
                ...
                <div className="val">{selected.orderNo || "-"}</div>
              </div>
              <div className="row">
              ...
              <div className="row">
                ...
                <div className="val">
                  {LABEL.cargoType[selected.cargoType] || selected.cargoType || "-"}
                </div>
              </div>
              <div className="row">
                <div className="key">크기</div>
                <div className="val">
                  {LABEL.cargoSize[selected.cargoSize] || selected.cargoSize || "-"}
                </div>
              </div>
              <div className="row">
                <div className="key">무게</div>
                <div className="val">
                  {LABEL.weight[selected.weight] || selected.weight || "-"}
                </div>
              </div>
              <div className="row">
                <div className="key">차량 종류</div>
                <div className="val">
                  {LABEL.vehicle[selected.vehicle] || selected.vehicle || "-"}
                </div>
              </div>
              <div className="row">
                <div className="key">포장 여부</div>
                <div className="val">
                  {prettyPacking(selected.packingOptions ?? selected.packingOption)}
                </div>
              </div>
              <div className="row">
              ...
              {isAssigned(selected, assignment) ? (
                <>
                  <div className="row">
                    ...
                    <div className="val">{selected?.assignedDriverNickname || "-"}</div>
                  </div>
                  <div className="row">
            ...
            <section className="section">
              ...
              {loadingOffers ? (
                ...
                <div className="offer-list">
                  {offers.map((o) => {
                    const driverDisplay = o.driverNickname || `기사 #${o.driverIdNum}`;
                    return (
                      <div key={o.id} className="offer-item">
                        <div>
                          <b>{driverDisplay}</b> · {Number(o.price ?? 0).toLocaleString()}원
              ...
              {userRole === "CARRIER" && canBid(selected, assignment) && (
                <>
                  ...
                  >
                    <button className="accept-btn" onClick={() => setBidOpen(true)}>
                      입찰 등록
                    </button>
                  </div>
            ...
            {userRole === "SHIPPER" &&
              ...
              selected?.driverPrice != null && (
                <section className="section" style={{ marginTop: 16 }}>
                  <button
                    className="accept-btn"
                    onClick={() => shipperMoveToPayment(selected)}
                  >
